...
class Event {
  ...
  getTagValue(key) {
    const array = this.getTagValues(key);
    if (!array.length)
      return null;

    if (array.length !== 1)
      throw new Error(`unexpected multiple values for ${key}: ${array}`);
...
class Relay {
  ...
  async open() {
    ...
    const waiter = new Promise((resolve, reject) => {
      ...
      this.socket.once('error', (err) => {
        console.log(`Error connecting to ${this.host}: ${err.message}\n`);
        reject(err);
      });
    });
    ...
    this.socket.on('error', (err) => {
      console.log(`Error from ${this.host}: ${err.message}\n`);
    });
  }
  ...
  async sendEvent(event) {
    const waiter = new Promise((resolve) => {
      ...
      const handler = (msg) => {
        console.log(`received from ${this.host}: ${msg.toString('utf8')}\n`);
        clearTimeout(timeout);
        resolve();
      };

      timeout = setTimeout(() => {
          console.log(`Error: ${this.host} timed out waiting for socket response\n`);
          if (this.socket)
